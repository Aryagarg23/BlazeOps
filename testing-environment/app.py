...
class FireRescueEnv:
    def __init__(self, grid_size=(10, 10), num_firefighters=3, num_civilians=5, wind_speed=5, fuel_moisture=0.2, slope=0):
    ...
    def render(self, steps=20, save_gif=False):
        ...
        for step in range(steps):
            ...
            if save_gif:
                images.append((colored_grid * 255).astype(np.uint8))  # Convert to 8-bit color
        
        plt.ioff()
        plt.show()
        
        if save_gif and images:
            imageio.mimsave('fire_simulation.gif', images, fps=2)
            print("GIF saved as 'fire_simulation.gif'.")

if __name__ == "__main__":
    env = FireRescueEnv()
    env.render(steps=50, save_gif=True)
